...
var Ready = {
  ...
  updateStatusItem: function(state) {
    switch (state) {
      case StateController.STATES.UNSUPPORTED:
        this.statusItem.setState("status-error", "Kite only supports macOS at the moment.");
        break;
      case StateController.STATES.UNINSTALLED:
        this.statusItem.setState("status-error", "Kite is not installed.");
        break;
      case StateController.STATES.INSTALLED:
        this.statusItem.setState("status-error", "Kite is not running.");
        break;
      case StateController.STATES.RUNNING:
        this.statusItem.setState("status-error", "Kite is running but not reachable.");
        break;
      case StateController.STATES.REACHABLE:
        this.statusItem.setState("status-error", "Kite is not authenticated.");
        break;
      case StateController.STATES.AUTHENTICATED:
        this.statusItem.setState("status-not-whitelisted", "Kite is not enabled for the current directory.");
        break;
      case StateController.STATES.WHITELISTED:
        this.statusItem.setState("status-ready", "Kite is ready.");
        break;
      default:
        this.statusItem.setState("status-unknown", `Kite is unknown state ${state}.`);
        break;
    }
  },
  ...
  onActivate: function() {
    ...
    this.statusItem.onClick(this.ensure.bind(this));
  },
