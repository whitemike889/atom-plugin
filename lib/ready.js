...
const StatusItem = require('./elements/kite-status.js');
...
var Ready = {
  ...
  updateStatusItem: function(state) {
    switch (state) {
      case StateController.STATES.UNSUPPORTED:
        this.statusItem.setState('error', 'Kite only supports macOS at the moment.');
        break;
      case StateController.STATES.UNINSTALLED:
        this.statusItem.setState('error', 'Kite is not installed.');
        break;
      case StateController.STATES.INSTALLED:
        this.statusItem.setState('error', 'Kite is not running.');
        break;
      case StateController.STATES.RUNNING:
        this.statusItem.setState('error', 'Kite is running but not reachable.');
        break;
      case StateController.STATES.REACHABLE:
        this.statusItem.setState('error', 'Kite is not authenticated.');
        break;
      case StateController.STATES.AUTHENTICATED:
        this.statusItem.setState('not-whitelisted', 'Kite is not enabled for the current directory.');
        break;
      case StateController.STATES.WHITELISTED:
        this.statusItem.setState('ready', 'Kite is ready.');
        break;
      default:
        this.statusItem.setState('unknown', `Kite is unknown state ${state}.`);
        break;
    }
  },
  ...
  // the problem.
    ...
    return StateController.handleState(curpath)
    .then(state => {
    ...
    .catch(err => metrics.track('handleState failed', err))
    .then(state => {
      if (state !== this.lastState && state === StateController.STATES.WHITELISTED) {
  ...
  currentPath: function() {
    ...
    return editor ? editor.getPath() || projectPath : projectPath;
  },
  ...
  shouldNotify: function(state) {
    ...
    return this.hasPythonFileOpen() && (prev === undefined || utils.secondsSince(prev) >= NOTIFY_DELAY);
  },
