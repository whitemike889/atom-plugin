...
const { KSGCodeBlocks } = require('../elements/ksg/kite-codeblocks');
...
module.exports = class KSG {
  ...
  shouldHideForTarget(target) {
    if (this.element.contains(target)) { return false; }
    while (target) {
      if (target instanceof KSGDefaultSearchResult || target instanceof KSGSearchResult) { return false; }
      if (target && target.classList && target.classList.contains('ksg-code-block-toggle')) { return false; }
