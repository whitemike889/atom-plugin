...
const parameterType = (p, prefix = '') =>
  p.inferred_value
    ? `${prefix}${uniq(compact(p.inferred_value).map(v =>
      `<a href="kite-atom-internal://type/${v.type_id}" class="parameter-type">${v.type}</a>`)).join('<span class="type-separator">&#8910;</span>')}`
    : '';
...
const returnTypes = v => {
  ...
  return types.length
    ? `<span class="return-types-container">${
      types
...
const appendCompletionsFooter = (completion, descriptionHTML) => {
  ...
  const actions = id
    ? `<a href="${internalExpandURL(id)}">Docs<span class="kite-link-icon">&#8663;</span></a>`
    : '';
