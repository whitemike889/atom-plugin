...
const parameterName = (p, prefix = '') =>
  p
    ? `${prefix}<a href="#" class="parameter-name">${p.name}</a>`
    : undefined;

const parameterType = (p, prefix = '') =>
  p.inferred_value
    ? `${prefix}<a href="#" class="parameter-type">${uniq(p.inferred_value.map(v => v.type)).join('|')}</a>`
    : '';
...
const valueLabel = value =>
  value.kind === 'function'
    ? valueName(value) + signature(value)
    : valueName(value);
...
const unionType = (vs, map) =>
  uniq(compact(flatten(vs).map(map))).map(wrapType).join(' | ');
