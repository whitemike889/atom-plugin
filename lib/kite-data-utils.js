...
const parameterName = (p, prefix = '', w) =>
  p
    ? (
      w
        ? `<${w}>${prefix}<span class="syntax--python"><span class="syntax--variable syntax--parameter syntax--function parameter-name">${p.name}</span></span></${head(w.split(/\s/g))}>`
  ...
  switch (lang) {
    ...
    case 'javascript':
      return detailNotEmpty(p, 'default_value')
        ? `<span class="syntax--python"><span class="syntax--keyword syntax--operator syntax--assignment">=</span></span>` +
        `<span class="syntax--python parameter-default">${highlightChunk(head(detailGet(p, 'default_value')).repr)}</span>`
        : '';
    default:
      return '';
  }
...
const valueLabel = (value, current) =>
  isFunctionKind(value.kind)
    ? highlightChunk(valueName(value) + '(') + "​" + // Zero-width space, allows text to wrap after the opening paren
