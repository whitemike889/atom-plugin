...
const parameterName = (p, prefix = '', w) =>
  p
    ? (
      w
        ? `<${w}>${prefix}<span class="syntax--variable syntax--parameter syntax--function syntax--python parameter-name">${p.name}</span></${head(w.split(/\s/g))}>`
        : `${prefix}<span class="syntax--variable syntax--parameter syntax--function syntax--python parameter-name">${p.name}</span>`
    )
    : undefined;
  ...
  switch (lang) {
    ...
    case 'javascript':
      return detailNotEmpty(p, 'default_value')
        ? `<span class="syntax--python syntax--keyword syntax--operator syntax--assignment">=</span><span class="syntax--python syntax-constant parameter-default">${head(detailGet(p, 'default_value')).repr}</span>`
        : '';
    default:
      return '';
  }
