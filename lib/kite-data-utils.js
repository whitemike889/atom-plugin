...
const wrapType = (t) =>
  t && t.type
    ? `<a href="kite-atom-internal://type/${t.type_id}" class="type-value">${t.type}</a>`
    : null;

const unionType = (vs, map) => {
  return uniq(compact(flatten(vs).map(map).map(wrapType))).join(' | ');
};
