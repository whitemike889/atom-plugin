...
const parameterDefault = (p) => {
  ...
  switch (p.language) {
    case 'python':
      return p.language_details &&
             p.language_details.Python.default_value &&
             p.language_details.Python.default_value.length
        ? `=<span class="parameter-default">${head(p.language_details.Python.default_value).repr}</span>`
        : '';
    case 'javascript':
      return p.language_details &&
             p.language_details.Javascript.default_value &&
             p.language_details.Javascript.default_value.length
        ? `=<span class="parameter-default">${head(p.language_details.Javascript.default_value).repr}</span>`
        : '';
    default:
      return '';
  }
  ...
  switch (detail.language) {
    case 'python':
      return [
        ...
        parameterName(detail.language_details.Python.vararg, '*'),
        parameterName(detail.language_details.Python.kwarg, '**', 'a class="kwargs" href="#"'),
      ];
    case 'javascript':
      return [
        parameters(detail, withType),
        parameterName(detail.language_details.Javascript.rest, '…'),
      ];
    default:
      return [
        parameters(detail, withType),
