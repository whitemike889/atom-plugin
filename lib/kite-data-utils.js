...
const parameterDefault = (p) =>
  p &&
  p.language_details &&
  p.language_details.default_value &&
  p.language_details.default_value.length
    ? `=<span class="parameter-default">${head(p.language_details.default_value).repr}</span>`
    : '';
  ...
  switch (detail.language) {
    case 'python':
      return [
        ...
        parameterName(detail.language_details.vararg, '*'),
        parameterName(detail.language_details.kwarg, '**', 'a class="kwargs" href="#"'),
      ];
    case 'javascript':
      return [
        parameters(detail, withType),
        parameterName(detail.language_details.rest, '…'),
      ];
    default:
      return [
        parameters(detail, withType),
