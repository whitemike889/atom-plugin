        ...
        if (!this.signature) {
          ...
          this.signature.onDidAttach = () => {
            ...
            this.scrollDisposable.add(new DisposableEvent(
              this.signature,
              'mousewheel', (e) => {
                if (this.signature.classList.contains('compact-mode')) {
                  e.stopPropagation();
                }
              }));
            this.scrollDisposable.add(view.onDidChangeScrollTop((e) => {
              this.requestOverflowCheck();
            }));
  ...
  requestOverflowCheck() {
    ...
    requestAnimationFrame(() => {
      ...
      if (this.marker) {
        ...
        const compacted = this.signature.classList.contains('compact-mode');

        const refHeight = this.referenceHeight || sigBounds.height;
        ...
        if (shouldFollowCompletion) {
          ...
          if (refHeight > availableSpace) {
            this.signature.classList.add('compact-mode');
            this.signature.style.height = `${availableSpace}px`;
          } else {
            this.signature.classList.remove('compact-mode');
            this.signature.style.height = '';
            delete this.referenceHeight;
          }
        } else {
          if (compacted) {
            this.signature.classList.remove('compact-mode');
          ...
          if (pixelPos.top - scrollTop < refHeight) {
            if (!upsideDown) {
