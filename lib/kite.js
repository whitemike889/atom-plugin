...
module.exports = {
  activate() {
    ...
    // April 2018
    atom.config.set('kite.searchIconPosition', atom.config.get('kite.activeSearchPosition'));
    atom.config.unset('kite.activeSearchPosition');

    // April 2018
    atom.config.set('kite.showWelcomeNotificationOnStartup', atom.config.get('kite.showDocsNotificationOnStartup'));
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor[data-grammar="source python"]', {
      'kite:search'() { Kite.getSearchItem().expand(); },
      'core:cancel'() {
        if (!OverlayManager) { OverlayManager = require('./overlay-manager'); }
        OverlayManager.dismiss();
      },
      'kite:docs-at-cursor'() { Kite.expandAtCursor(); },
      'kite:open-in-web'() {
        const editor = atom.workspace.getActiveTextEditor();
