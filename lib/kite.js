...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors(editor => {
      // We only want to check the state for supported languages
      if (this.isGrammarSupported(editor)) {
        this.app.checkPath(editor.getPath()).then(state => {
          // we only subscribe to the editor if it's in a
    ...
    this.subscriptions.add(atom.workspace.onDidChangeActivePaneItem(item => {
      if (item instanceof TextEditor) {
        this.app.checkPath(item.getPath());
      }
    }));
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor', {
      ...
      'kite:open-in-web-at-cursor'() {
        const editor = atom.workspace.getActiveTextEditor();
        const position = editor.getLastCursor().getBufferPosition();
        OverlayManager.openInWebAtPosition(editor, position);
      },
    }));
