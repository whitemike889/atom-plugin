...
const KiteExpandModule = require('./elements/kite-expand-module');
const KiteExpandFunction = require('./elements/kite-expand-function');
...
const KiteMembersList = require('./elements/kite-members-list');
...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(scheme.onDidClickLink(({href, host, path}) => {
      console.log(`link to ${href} clicked`);

      switch (host) {
        case 'members-list': {
          const id = path.replace(/^\//, '');
          const key = `${id} members`;

          if (this.sidebar.breadcrumb.includes(key)) {
            this.sidebar.goto(key);
          } else {
            DataLoader.getValueReportDataForId(id).then(data => {
              const element = new KiteMembersList();
              element.setData(data);

              this.sidebar.stack(key, element);
            });
          }
        ...
        case 'member': {
          ...
          DataLoader.getValueReportDataForId(id).then(data => {
            ...
            this.sidebar.stack(data.value.repr, element);
          });
        ...
        case 'sidebar-depth': {
          ...
          if (step === 'previous') {
            this.sidebar.previous();
          } else {
            this.sidebar.goto(parseInt(step, 10));
          }
        }
      }
    }));
