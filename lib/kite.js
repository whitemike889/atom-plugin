...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(this.app.onKiteReady(() => {
      ...
      .forEach(e => {
        if (this.hasEditorSubscription(e)) {
          this.kiteEditorForEditor(e).updateTokens();
        } else {
          this.subscribeToEditor(e);
        }
      });
    }));
  ...
  handle403Response(editor, resp) {
    const previousStatus = this.isEditorWhitelisted(editor);
    const status = this.getStatusItem();
    if (resp.statusCode === 403 && previousStatus) {
      this.kiteEditorForEditor(editor).tokensList.clearTokens();
      delete this.whitelistedEditorIDs[editor.id];
      if (status.lastState === KiteApp.STATES.WHITELISTED) {
        status.setState(KiteApp.STATES.AUTHENTICATED);
      }
    } else if (!previousStatus) {
      this.whitelistedEditorIDs[editor.id] = true;
      this.kiteEditorForEditor(editor).updateTokens();
      if (status.lastState !== KiteApp.STATES.WHITELISTED) {
        status.setState(KiteApp.STATES.WHITELISTED);
      }
    }
  },
