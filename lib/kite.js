...
module.exports = {
  activate: function() {
      ...
      this.subscriptions.add(atom.workspace.observeTextEditors(editor => {
        if (/\.py$/.test(editor.getPath() || '')) {
          ...
          subs.add(expandGesture.onDidActivate(range => {
            OverlayManager.showExpandAtRange(editor, range);
          }));
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'kite:show-expanded-function-panel'() { appendExpand('function'); },
      'kite:show-expanded-instance-panel'() { appendExpand('instance'); },
      'kite:show-expanded-module-panel'() { appendExpand('module'); },
    }));

    function appendExpand(type) {
