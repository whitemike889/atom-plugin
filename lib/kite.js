...
const HoverManager = require('./hover-manager');
    ...
    this.subscriptions.add(ready.onKiteReady(() => {
      this.subscriptions.add(atom.workspace.observeTextEditors(editor => {
        if (/\.py$/.test(editor.getPath() || '')) {
          ...
          const gesture = new HoverGesture(editorElement);
          const subs = new CompositeDisposable();

          subs.add(gesture);

          subs.add(gesture.onDidActivate(screenPosition => {
            HoverManager.showHoverAtPosition(editor, screenPosition);
          }));

          subs.add(gesture.onDidDismiss(screenPosition => {
            HoverManager.dismissHover();
          }));
