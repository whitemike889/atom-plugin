...
let child_process, CompositeDisposable, TextEditor, Range, AccountManager,
  ...
  DisposableEvent, KiteActiveSearch, EditorEvents, BrowserClient,
  addDelegatedEventListener, AUTOCORRECT_SHOW_SIDEBAR;
...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'kite:open-error-rescue-sidebar': () => this.toggleAutocorrectSidebar(true),
      'kite:close-error-rescue-sidebar': () => this.toggleAutocorrectSidebar(false),
      'kite:toggle-error-rescue-sidebar': () => this.toggleAutocorrectSidebar(),
    }));
  ...
  createGlobalTextEditorStyleSheet() {
    ...
    const listener = () => {
      ...
      console.warn('IM LISTENING')

      let el = document.createElement('atom-text-editor');
      document.body.appendChild(el);
      let editorFgColor = window.getComputedStyle(el, null).getPropertyValue("background-color");
      console.warn(editorFgColor);
      el.parentNode.removeChild(el);

      let el2 = document.createElement('div');
      el2.classList.add('atom-dock-mask');
      document.body.appendChild(el2);
      let paneBkColor = window.getComputedStyle(el2, null).getPropertyValue("background-color");
      console.warn(paneBkColor);
      el2.parentNode.removeChild(el2);

      function calculateContrast(colorString) {
        // colorString is guaranteed to be "rgb(r, g, b)" or "rgba(r, g, b, a)"
        let match = colorString.match(/rgba?\(([\d]*?), ([\d]*?), ([\d]*?)(, ([\d\.]*?))?\)/)
      ...
      console.warn('-----')
      let editorFgContrast = calculateContrast(editorFgColor)
      let paneBkContrast = calculateContrast(paneBkColor)
    ...
    atom.config.observe('core.themes', listener);
  },
