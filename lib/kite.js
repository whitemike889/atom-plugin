...
module.exports = {
    ...
    this.subscriptions.add(atom.workspace.onDidChangeActivePaneItem(item => {
      if (item instanceof TextEditor) {
        this.checkTextEditor(item).then(([state, supported]) => {
          this.getStatusItem().setState(state, supported);
        });
      }
    }));
  ...
  patchCompletions() {
    ...
    .then(autocompletePlus => {
      ...
      const list = manager && atom.views.getView(manager.suggestionList);
      const safeUpdate = list && list.updateDescription;

      if (safeUpdate) {
        list.updateDescription = (item) => {
          safeUpdate.call(list, item);

          if (!item) {
            if (list.model && list.model.items) {
              item = list.model.items[list.selectedIndex];
            }
          }

          if (list.descriptionContainer.style.display === 'none' &&
              item && item.descriptionHTML) {
            list.descriptionContainer.style.display = 'block';
            list.descriptionContainer.classList.add('kite-completions');
            list.descriptionContent.innerHTML = item.descriptionHTML;
            list.setDescriptionMoreLink(item);
          }
        };

        if (!list.ol) { list.renderList(); }
      } else {
        list.descriptionContainer.classList.remove('kite-completions');
      }
    });
  },
