...
const OUTPUTS = {
  ...
  file: part =>
    `<figcaption>${part.content.caption}</figcaption>
    <pre class="input"><code>${atob(part.content.data)}</code></pre>`,
};
...
class KiteCuratedExample extends HTMLElement {
  ...
  setData(data) {
    ...
    const inputHTML = inputFiles && inputFiles.length
      ...
      ${inputFiles.map(f => {
        return `<figcaption>${f.name}</figcaption><pre class="input"><code>${atob(f.contents_base64)}</code></pre>`;
      }).join('')}
      `
      : '';
    this.innerHTML = `<div class="example-wrapper">${html}${inputHTML}</div>`;
  }
}
