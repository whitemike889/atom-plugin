...
const fileEntry = file =>
  `<li class='list-item'>
      <span class='icon icon-file-text' data-name="${file.name}">${file.name}</span>
  </li>`;
...
const dirEntry = dir => {
  ...
  <li class='list-nested-item'>
    <div class='list-item'>
      <span class='icon icon-file-directory' data-name="${dir.name}">${dir.name}</span>
    </div>
...
function processInput(part) {
  const [handler] = INPUTS.filter(([predicate]) => predicate(part))[0];
  return handler(part);
}
...
class KiteCuratedExample extends HTMLElement {
  ...
  setData(data) {
    ...
    this.innerHTML = `
      <div class="expand-header"><span class="name">${data.title}</span></div>
