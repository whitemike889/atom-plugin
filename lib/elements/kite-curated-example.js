...
const processReferences = (line, references, highlighter) => {
  const res = references.reduce((o, ref) => {
    ...
    if (ref.begin >= o.start && ref.end <= o.end) {
      ...
      console.log('pre', prefix, 'ref', reference, 'post', postfix);
    }
    return o;
  }, {
    plain: [],
  ...
  setData(data) {
    ...
    .map(part => {
      if (part.type === 'code') {
        ...
        if (part.output_type === 'image') {
          return `
          <h5>${part.content.path}</h5>
          <img src="data:;base64,${part.content.data}" title="${part.content.path}">
          `;
        } else if (part.output_type === 'plaintext') {
          return `<pre class="output"><code>${part.content.value}</code></pre>`;
        } else {
          return '';
        }
      } else {
