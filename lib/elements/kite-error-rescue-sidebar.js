      ...
      <div class="kite-column">
        <div class="header">
          ...
          <div class="settings-view">
            <a href="https://help.kite.com/article/78-what-is-error-rescue" title="Error Rescue automatically fixes the most common mistakes when you save your file.<br><br>Click to learn more." class="learn-more icon icon-question"></a>
      ...
      const diffsHTML = history.map((fix, index) => {
        ...
        html += `
          ...
          <kite-links class="one-line feedback-actions">
            <a class="go-to-code icon icon-file-code" href="kite-atom-internal://goto/${encodeURI(stripLeadingSlash(editor ? editor.getPath() : filename))}:${row + 1}:${column + 1}"
  ...
  renderDiff(diff) {
    return [
      ...
      (diff.inserted || diff.new).map(ins => `<ins>
          ...
          <div class="line">${highlightChunk(ins.text)}</div>
        </ins>`).join(''),
      '</code>',
    ].join('');
