...
function debugData(data) {
  return atom.config.get('kite.developperMode')
    ? `<div class="debug">
...
function renderUsages(symbol) {
  return symbol.report && symbol.report.usages && symbol.report.usages.length
    ? section('Usages',
      Plan.can('usage_editor')
        ? `<ul class="usages-box">
          ${symbol.report.usages.map(renderUsage).join('')}
        </ul>`
        : `<div class="kite-pro-features">
            To see ${symbol.report.usages.length} usages,
            <a href="http://kite.com/pro">upgrade to Kite Pro</a>, or
            <a href="http://kite.com/refer">get Kite Pro for free</a>
          </div>`)
    : '';
}
...
function renderUsage(usage) {
  ...
  return `<li class="usage">
    ...
    <div class="links">
      <a href="${url}">${base}:${usage.line + 1}</a>
    </div>
  </li>`;
}
