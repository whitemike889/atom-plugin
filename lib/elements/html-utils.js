...
function renderMembers(value, limit) {
  ...
  return members.length === 0
    ...
    : (limit != null
      ...
      : section('Top members', `<dl>${members.map(m => renderMember(m)).join('')}</dl>`));
}
...
function additionalMembersLink(membersCount, value) {
  return membersCount <= 0
    ? ''
    : `<a href="kite-atom-internal://members-list/${value.id}" class="more-members">See ${membersCount} more methods</a>`;
}
