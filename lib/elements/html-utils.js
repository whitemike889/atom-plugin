...
function renderExamples(value, limit = 2) {
  return value.report && value.report.examples && value.report.examples.length
    ? section('Examples', `
      <ul>${value.report.examples.slice(0, limit).map(renderExample).join('')}</ul>
      ${additionalExamplesLink(value.report.examples.length - 2, value)}
    `)
    : '';
}
...
function additionalExamplesLink(examplesCount, value) {
  return examplesCount <= 0
    ? ''
    : `<a href="kite-atom-internal://examples-list/${value.id}"
