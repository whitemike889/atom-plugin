...
function proFeatures(message) {
  return Plan.hasStartedTrial()
    ? `<div class="kite-pro-features">
      ${message},
      <a href="http://localhost:46624/redirect/pro">upgrade to Kite Pro</a>, or
      <a href="http://localhost:46624/redirect/invite">get Kite Pro for free</a>
      </div>`
    : `${message},
      <a href="http://localhost:46624/redirect/trial">start your Kite Pro trial</a> at any time`;
}
...
// Helps with intentional text wrapping. HTML is not very well equipped to break
// code strings in natural places. Instead of allowing word-break to do it
// haphazardly, we’ll insert zero-width spaces in all the right places – the
// same places a programmer might choose to break to a new line.
function addBetterWrapping(content) {
...
function renderUsages(symbol) {
  return symbol.report && symbol.report.usages && symbol.report.usages.length
    ? section('Examples from your code',
      Plan.can('usages_editor') /* False */
        ? `<ul class="usages-box">
          ${symbol.report.usages.map(renderUsage).join('')}
        </ul>`
        : proFeatures(`To see ${symbol.report.usages.length} ${pluralize(symbol.report.usages.length, 'example', 'examples')}`),
      { help: '<a href="https://help.kite.com/article/58-why-kite-is-cloud-powered" title="This section is visible just to you.<br><br>Click to learn more." class="learn-more icon icon-question"></a>'} )
    : '';
}
...
function renderUsage(usage) {
  ...
  return `<div class="usage-container">
    <li class="usage">
      ${highlightCode(addBetterWrapping(usage.code.trim()))}
      <div class="links">
        <i class="icon icon-file-code"></i>
  ...
  return members.length === 0
    ...
    : (limit != null
      ? section('Popular members', `
        ...
        ${additionalMembersLink(total_members - limit, value, limit)}`)
      : section('Popular members', `
        <ul class='code'>
          ${members.map(m => renderMember(m)).join('')}
        </ul>
        ${total_members > members.length
          ? additionalMembersLink(total_members, value, limit)
          : ''
        }`));
}
