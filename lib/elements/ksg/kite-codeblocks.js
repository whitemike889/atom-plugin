...
class KSGCodeBlockGroup extends HTMLElement {
  constructor(query, position, answer = {}, isDefaultGroup = false) {
    ...
    if (this.isDefaultGroup) {
      ...
      this.subscriptions.add(defaultBlock.onClicked(({ query }) => {
      ...
      if (code_blocks && code_blocks.length > 0) {
        ...
        code_blocks.forEach((block, idx) => {
          const blockEl = idx == 0 ?
            new KSGCodeBlock(block, question_title, answer_id, votes, is_accepted) :
            new KSGCodeBlock(block);
...
class KSGDefaultCodeBlock extends HTMLElement {
  ...
  clickHandler(e) {
    const query = !this.query.toLowerCase().includes('python') ?
      `python ${this.query}` : this.query;
