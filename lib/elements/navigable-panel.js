...
const KiteExpandUnknown = require('./kite-expand-unknown');
...
class NavigablePanel extends HTMLElement {
  ...
  setData([hover, report], noMetrics = false) {
    ...
    switch (kind) {
      ...
      case 'type':
        ...
        break;
      default:
        element = new KiteExpandUnknown();
        element.setData(atom.config.get('kite.developerMode') ? data : '');
    }

    let els = element.querySelectorAll('.learn-more');
    for (var i = 0, el; el = els[i]; i++) {
      this.subscriptions.add(atom.tooltips.add(el, { title: () => '' }));
    }
  ...
  showDataAtPosition(editor, position, noMetrics = false) {
    ...
    .then(() => DataLoader.getReportDataAtPosition(editor, position))
    .then(([hover, report]) => {
      ...
      // This means invoking the “show docs at cursor” failed.
      // We beep and in case the expand at cursor highlight is still
      // visible, we turn it red
      atom.beep();

      let expandAtCursorHighlight = Kite.getLastExpandAtCursorHighlight();
      if (expandAtCursorHighlight) {
        expandAtCursorHighlight.indicateError();
      }
    });
  }
