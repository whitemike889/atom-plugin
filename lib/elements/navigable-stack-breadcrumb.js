...
class NavigableStackBreadcrumb extends HTMLElement {
  ...
  createdCallback() {
    ...
    this.innerHTML = `<div class="btn-group">
      <a href="kite-atom-internal://navigation-depth/previous" class="btn btn-link icon icon-chevron-left"></a>
      <a href="kite-atom-internal://navigation-depth/next" class="btn btn-link icon icon-chevron-right"></a>
    </div>
  ...
  updateBreadcrumb() {
    if (this.breadcrumb.length > 1) {
      this.breadcrumbList.innerHTML = this.breadcrumb.map((b, i, a) =>
        `<li class="item-${i}"><a href="kite-atom-internal://navigation-depth/${i}">${b.split(';').pop()}</a></li>`
      ).join('');
