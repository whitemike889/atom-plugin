...
class KiteStatusPanel extends HTMLElement {
  ...
  handleAction({url}) {
    ...
    switch (url.host) {
      ...
      case 'start-enterprise':
        ...
        break;
      case 'login':
        this.pendingStatus('Sign in to Kite');
  ...
  renderLinks(account) {
    ...
    <ul class="links ${account ? 'has-account' : 'no-account'}">
      ${giftLink}
      <li><a href="http://localhost:46624/clientapi/desktoplogin?d=/docs" class="account-dependent">Kite Web Search</a></li>
    </ul>
    <ul class="links ${account ? 'has-account' : 'no-account'}">
      ...
      <li><a href="http://localhost:46624/settings/permissions" class="account-dependent">Kite Permissions</a></li>
