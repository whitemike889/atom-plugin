...
class KiteStatusPanel extends HTMLElement {
  ...
  renderLinks(account) {
    ...
    <ul class="links ${account ? 'has-account' : 'no-account'}">
      ${giftLink}
      <li><a href="http://localhost:46624/clientapi/desktoplogin?d=/docs" class="account-dependent">Search Python Documentation</a></li>
    </ul>
    <ul class="links ${account ? 'has-account' : 'no-account'}">
      <li><a is="kite-localtoken-anchor" href="atom://settings-view/show-package?package=kite">Atom Plugin Settings</a></li>
      <li><a is="kite-localtoken-anchor"
           class="account-dependent"
           href="kite://settings">Kite Settings</a></li>
      <li><a is="kite-localtoken-anchor" 
             href="kite://settings/permissions" 
             class="account-dependent">Kite Permissions</a></li>

      <li><a href="http://help.kite.com/category/43-atom-integration">Help</a></li>
    </ul>
    `;
  }
    ...
    switch (status.state) {
      ...
      case STATES.AUTHENTICATED:
        ...
        if (!editor) {
          ...
          const settingsURL = `kite://settings/permissions?filename=${path}`;
          content = shouldOfferWhitelist
            ? `<div class="text-warning">Kite engine is not enabled for this file ${dot}</div>
              ...
              <a href="${settingsURL}"
                 ...
                 class="btn btn-warning">Whitelist settings…</a>`
            : `<div>The current file is ignored by Kite ${dot}</div>
              <a href="${settingsURL}"
                 is="kite-localtoken-anchor"
                 class="btn btn-default">Whitelist settings…</a>`;
        }
