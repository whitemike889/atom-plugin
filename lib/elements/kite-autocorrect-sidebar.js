...
const {DisposableEvent, stopPropagationAndDefault, addDelegatedEventListener} = require('../utils');
...
class KiteAutocorrectSidebar extends HTMLElement {
  ...
  attachedCallback() {
    ...
    this.subscriptions.add(addDelegatedEventListener(this, 'click', '.thumb-up', () => {
      DataLoader.postAutocorrectFeedbackData(this.getCurrentEditorLastCorrections(), +1);
    }));

    this.subscriptions.add(addDelegatedEventListener(this, 'click', '.thumb-down', () => {
      DataLoader.postAutocorrectFeedbackData(this.getCurrentEditorLastCorrections(), -1);
    }));
