...
class KiteSignature extends HTMLElement {
  ...
  disconnectedCallback() {
    ...
    this.popularPatternsExpanded = false;
  }
  ...
  setData(data, compact = false) {
    ...
    if (!compact) {
      ...
      if (call.signatures && call.signatures.length) {
        patterns = `
          ...
          <h4><span class="title">How others used this<span class="popular-patterns-toggle">${ this.popularPatternsExpanded ? '&#8613;' : '&#8615;' }</span></span></h4>
      ...
      const actions = [
        `<a class="kite-link patterns" href="#">Patterns<span class="kite-link-icon">&#8615;</span></a>
        <a class="kite-link docs" href="${internalExpandURL(call.callee.id)}">Docs<span class="kite-link-icon">&#8663;</span></a>`,
      ];
    ...
    const popularPatternsLink = this.querySelector('a.kite-link.patterns');
    ...
    if (popularPatternsSection && popularPatternsToggle && popularPatternsTitle) {
      this.subscriptions.add(new DisposableEvent(popularPatternsTitle, 'click', (e) => {
        this.popularPatternsExpanded = !this.popularPatternsExpanded;
        popularPatternsSection.classList.toggle('visible');
        this.popularPatternsExpanded 
          ? popularPatternsToggle.textContent = '\u21a5' 
          : popularPatternsToggle.textContent = '\u21a7';
      }));
    }

    console.log('hhjhjhh', popularPatternsLink, popularPatternsSection)
    if (popularPatternsLink && popularPatternsSection) {
      console.log('we whoa')
