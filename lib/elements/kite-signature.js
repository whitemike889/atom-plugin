...
const {highlightCode, addBetterWrapping, debugData, proFeatures, pluralize, renderParameter} = require('./html-utils');
  ...
  setData(data, compact = false) {
    ...
    if (!compact) {
      ...
      if (call.signatures && call.signatures.length) {
        patterns = Plan.can('common_invocations_editor') /* False */
          ? `
            <section class="patterns">
            <h4>How others used this</h4>
            ${highlightCode(addBetterWrapping(
              call.signatures
      ...
      const actions = [
        `<a href="${openSignatureInWebURL(call.callee.id)}">Web</a>`,
