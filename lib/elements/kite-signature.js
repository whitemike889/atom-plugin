...
class KiteSignature extends HTMLElement {
  ...
  setData(data, compact = false) {
    ...
    if (detail.language === 'python' &&
        detail.language_details.kwarg_parameters &&
        detail.language_details.kwarg_parameters.length) {
      kwargs = `<section class="kwargs ${call.language_details.in_kwargs ? 'visible' : ''}">
      <h4>**${detail.language_details.kwarg.name}</h4>
      <kite-links metric="Signature">
      <dl>
      ${
        detail.language_details.kwarg_parameters
        .map((p, i) => renderParameter(p, '', call.language_details.in_kwargs && call.arg_index === i))
        .join('')
      }
      </dl></kite-links>
      </section>`;
    ...
    <div class="kite-signature-wrapper">
      <div class="name">${
        valueLabel(
          call.callee, call.language_details && call.language_details.in_kwargs
            ? -1
            : call.arg_index
        )
      }</div>
