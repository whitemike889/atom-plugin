...
class KiteSignature extends HTMLElement {
  ...
  setData(data, compact = false) {
    ...
    let kwargs = '';
    if (detail.language === 'python' &&
        detail.language_details.Python.kwarg_parameters &&
        detail.language_details.Python.kwarg_parameters.length) {
      kwargs = `<section class="kwargs ${call.language_details.Python.in_kwargs ? 'visible' : ''}">
      <h4>**${detail.language_details.Python.kwarg.name}</h4>
      <kite-links metric="Signature">
      <dl>
      ${
        detail.language_details.Python.kwarg_parameters
        .map((p, i) => renderParameter(p, '', call.language_details.Python.in_kwargs && call.arg_index === i))
        .join('')
      }
      </dl></kite-links>
      </section>`;
    ...
    <div class="kite-signature-wrapper">
      <div class="name">${
        valueLabel(
          call.callee, call.language_details && call.language_details.Python && call.language_details.Python.in_kwargs
            ? -1
            : call.arg_index
        )
      }</div>
    ...
    if (kwargLink && kwargSection) {
      ...
      if (call.language_details.Python && call.language_details.Python.in_kwargs) {
        setTimeout(() => {
