...
class KiteStatus extends HTMLElement {
  ...
  attachedCallback() {
    ...
    this.innerHTML = '<kite-logo small></kite-logo><kite-logo sync></kite-logo><span class="text">Kite</span>';
  ...
  setStatusLabel(kiteEditor) {
    ...
    if (kiteEditor) {
      ...
      // We store the hover request promise and compare the previous cursor
      // position with the last one to only make new requests
      const promise = this.lastDocsAtCursorPromise && String(pos) === this.lastDocsAtCursorPosition
        ? this.lastDocsAtCursorPromise
        : DataLoader.getHoverDataAtPosition(kiteEditor.editor, pos);

      this.lastDocsAtCursorPosition = String(pos);
