...
const {StateController, AccountManager, Logger, Installation, Installer, DecisionMaker, Errors, Metrics} = require('kite-installer');
...
class KiteApp {
  ...
  connect() {
    return StateController.handleState().then(state => {
      ...
      return Plan.queryPlan().then(plan => {
        return state;
      });
    });
  }
  ...
  getRootDirectory(editor) {
    ...
    return path.relative(os.homedir(), basepath).indexOf('..') === 0
