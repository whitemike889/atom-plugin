...
let os, path, Emitter, StateController, AccountManager, Logger, Installation,
  Installer, DecisionMaker, Errors, Metrics, KiteLogin, Plan, parseJSON,
  promisifyReadResponse, promisifyRequest, localconfig, errors, flatten, DataLoader;

const ensureKiteDeps = () => {
  if (!StateController) {
    ({
      StateController, AccountManager, Logger, Installation, Installer, DecisionMaker, Errors, Metrics,
...
const ensureUtils = () => {
  if (!promisifyRequest) {
    ({parseJSON, promisifyReadResponse, promisifyRequest, flatten} = require('./utils'));
  }
};
...
class KiteApp {
  ...
  getSupportedLanguagesRegExp(languages) {
    ensureUtils();
    return `\.(${
      flatten(languages.map(l => EXTENSIONS_BY_LANGUAGES[l])).join('|')
    })$`;
  }
}
