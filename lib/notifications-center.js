...
class NotificationsCenter {
  ...
  warnNotRunning(state) {
    Promise.all([
      ...
      StateController.isKiteEntrepriseInstalled().then(() => true).catch(() => false),
    ]).then(([kiteInstalled, kiteEntrepriseInstalled]) => {
      if (kiteInstalled && kiteEntrepriseInstalled) {
        this.instrumentNotification(atom.notifications.addWarning(
          'The Kite engine is not running', {
            ...
            buttons: [{
              ...
              text: 'Start Kite Entreprise',
              metric: 'startEntreprise',
              onDidClick: dismiss => {
                dismiss();
                this.app && this.app.startEntreprise().catch(err => this.warnFailedStartEntreprise(err));
              },
            }],
          }));
      ...
      } else if (kiteEntrepriseInstalled) {
        this.instrumentNotification(atom.notifications.addWarning(
          'The Kite engine is not running', {
            metric: state,
            description: 'Start the Kite background service to get Python completions, documentation, and examples.',
  ...
  warnFailedStartEntreprise(err) {
    console.log('warnFailedStartEntreprise');
    this.instrumentNotification(atom.notifications.addError(
      'Unable to start Kite engine', {
        metric: 'launchEntreprise',
        description: JSON.stringify(err),
        dismissable: true,
        buttons: [{
          text: 'Retry',
          metric: 'retryEntreprise',
          onDidClick: dismiss => {
            dismiss();
            console.log('here');
            this.app && this.app.startEntreprise().catch(err => this.warnFailedStartEntreprise(err));
