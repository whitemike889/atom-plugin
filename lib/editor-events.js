...
const {last, DisposableEvent} = require('./utils');
let Kite;
...
class EditorEvents {
  ...
  mergeEvents() {
    ...
    .then(KiteAPI.emitWhitelistedPathDetected(event.filename))
    .catch(KiteAPI.emitNonWhitelistedPathDetected(event.filename));
  }
