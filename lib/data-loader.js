...
const DataLoader = {
  shouldOfferWhitelist(editor) {
    ...
    .then(resp => {
      ...
      if (resp.statusCode === 403) {
        ...
        return promisifyReadResponse(resp).then(path =>
          this.shouldNotify(editor)
          .then(res => {
            return res
              ? path
              : null;
  ...
  appendCompletionsFooter(completion, descriptionHTML) {
    return `
      ...
      <div class="kite-completion-footer text-right">
        <a href="kite-atom-internal://definition/path:line">def</a>
        <a is="kite-localtoken-anchor" href="#">web</a>
        <a href="kite-atom-internal://expand/id">more</a>
        <kite-logo></kite-logo>
      </div>
    `;
  },
  ...
  openInWebForId(id) {
    atom.applicationDelegate.openExternal(openDocumentationInWebURL(id, true));
  },
};
