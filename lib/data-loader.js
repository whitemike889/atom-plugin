...
const DataLoader = {
  ...
  getSnippetCompletionsAtPosition(editor) {
    ...
    return KiteAPI.getSnippetCompletionsAtPosition(editor.getPath(), text, 'atom', start, end)
    .then(completions => {
      ...
      completions.forEach(function(c) {
        ...
        if (c.children) {
          c.children.forEach(function(child) {
            parsedCompletions.push(parseSnippetCompletion(editor, child, String.fromCharCode(0x00A0).repeat(4)));
          });
        }
      });
  ...
  getKSGCodeBlocks(query) {
    return KiteAPI.getKSGCodeBlocks(query);
  },
