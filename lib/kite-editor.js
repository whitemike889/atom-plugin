...
const {utils: KiteUtils} = require('kite-installer');
...
class KiteEditor {
  constructor(editor) {
    ...
    KiteUtils.retryPromise(() => this.updateTokens(), 5, 200);
  }
  ...
  updateTokens() {
    return DataLoader.getTokensForEditor(this.editor).then(tokens => {
