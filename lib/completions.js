...
const KiteProvider = {
  ...
  getSuggestions(params) {
    ...
    console.log('here', Kite.isEditorWhitelisted(params.editor));
    ...
    return Kite.isEditorWhitelisted(params.editor) && Kite.app.isGrammarSupported(params.editor)
      ? delayPromise(() => {
        ...
        if (this.isInsideFunctionCall(params) && /[\(,=]$/.test(prefixChar)) {
        ...
        return promise.then(() =>
        DataLoader.getCompletionsAtPosition(params.editor, position));
      }, 0)
      : Promise.resolve();
  },
