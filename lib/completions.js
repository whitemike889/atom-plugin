  ...
  getSuggestions(params) {
    ...
    return editors.isGrammarSupported(params.editor)
      ? delayPromise(() => {
        ...
        // We abuse the activatedManually data to track snippet triggers.
        // See kite.js (calls to AC+'s findSuggestion function).
        let trigger = null;
        if (params.activatedManually === 'snippet') {
            trigger = 'snippet';
        } else if (params.activatedManually) {
            trigger = 'manual';
        } else {
            trigger = 'automatic';
        }
        ...
        return promise.then(() =>
          DataLoader.getCompletions(params.editor, atom.config.get('kite.enableSnippets'), trigger))
          .then(suggestions =>
